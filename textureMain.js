...
function setUpTextures(){
    ...
    // set up additional image texture
    imageTexture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, imageTexture);

    var imageImage = document.getElementById('image-texture')
    imageImage.crossOrigin = "";

    gl.bindTexture(gl.TEXTURE_2D, imageTexture);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, imageImage.width, imageImage.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, imageImage);

    // set up procedural image texture
    procTexture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, procTexture);

    var procImage = document.getElementById('image-texture')
    procImage.crossOrigin = "";

    // procedurally change image-texture with cool gradient
    for (let i = 0; i < procImage.width * procImage.height; i++) {
        // vec 4 for rgba pixel
        let pixel = [1 / i, 1 / i, 1 / i, 1 / i]
        procImage[i] = pixel;
    }
